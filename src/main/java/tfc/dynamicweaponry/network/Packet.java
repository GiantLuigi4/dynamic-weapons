...
public class Packet implements net.minecraft.network.protocol.Packet {
	...
	public void respond(NetworkEvent.Context ctx, Packet packet) {
		ctx.enqueueWork(() -> {
			if (checkClient(ctx)) DynamicWeaponryNetworkRegistry.NETWORK_INSTANCE.sendToServer(packet);
			else DynamicWeaponryNetworkRegistry.NETWORK_INSTANCE.send(PacketDistributor.PLAYER.with(ctx::getSender), packet);
		});
	}
}
